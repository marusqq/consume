...
- [x] Set up environment variable loading for LLM API key configuration
  ...
  - Created `.env.example` documenting `ANTHROPIC_API_KEY` as required (with placeholder value) and `CON...
...
- [x] Implement `cli.py` with argument parsing: positional `<url>` arg and optional `--mode short|long` flag
  ...
  - Added `parse_args()` function to `consume/cli.py` that creates an `ArgumentParser` with a `url` posi...
...
- [x] Wire up the fetch → extract → summarize → output pipeline in `cli.py`
  ...
  - Created `consume/summarizer.py` with a `summarize(text, mode)` function calling Claude Haiku (`claud...
  ...
  - Added error handling for `AuthenticationError`, `RateLimitError`, `APITimeoutError`, and generic `AP...
  - Added `CONSUME_MODEL` environment variable override for the model used
